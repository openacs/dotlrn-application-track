...
ad_library {
    ...
    Procs to set up the dotLRN application-track applet
    
    @author David Ortega (doa@tid.es)

}

namespace eval dotlrn_application_track {
    
    ad_proc -public applet_key {
    } {
        What's my applet key?
    } {
        return "dotlrn_application_track"
    }
	...
	return "dotlrn-application-track"
    }
	...
	Add the application_track applet to dotlrn. One time init - must be repeatable!
    } {
	...
	Add the application_track applet to a specific dotlrn community
    } {
	set portal_id [dotlrn_community::get_portal_id -community_id $community_id]

	# create the application_track package instance (all in one, I've mounted it)
	...
	application_track_admin_portlet::add_self_to_page \
	...
	set clone_courses [db_list_of_lists course_clone {
	    SELECT man_id, application_track_instance_id, community_id, class_key, isenabled, istrackable 
	...
	if {![empty_string_p $clone_courses]} {
		...
		db_dml add-course {
			...
			(man_id, application_track_instance_id, community_id, class_key, isenabled, istrackable) \
			values \
